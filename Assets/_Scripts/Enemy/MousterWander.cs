...
public class MousterWander : MonoBehaviour {
	...
	void Update () {
		...
		if (stateInfo.fullPathHash == Animator.StringToHash ("Base Layer.Stand")) {
			if (Time.time - lastActTime > actRestTime) {
				RandomAction ();         // 随机切换指令  
			}
			// 该状态下的检测指令  
			EnemyDistanceCheck ();
		} else if (stateInfo.fullPathHash == Animator.StringToHash ("Base Layer.Check")) {
			...
			// 该状态下的检测指令  
			ReturnCheck ();  
		} else if (stateInfo.fullPathHash == Animator.StringToHash ("Base Layer.Attack")) {
